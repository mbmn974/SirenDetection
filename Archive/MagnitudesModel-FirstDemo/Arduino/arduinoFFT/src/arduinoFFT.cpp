...
// vM is half the size of vReal and vImag
#warning("This method is deprecated and may be removed on future revisions.")
//ajout du log et de la multiplication par 20 - Modif
  for (uint16_t i = 0; i < samples; i++) {
...
void arduinoFFT::Windowing(FFTWindow windowType, FFTDirection dir) {
  ...
  //ajout d'enregistrement des poids de la fenêtre d'analyse et de la somme du buffer pour la normaliser
  double windowBuffer[this->_samples >> 1];
  ...
  for (uint16_t i = 0; i < (this->_samples >> 1); i++) {
	...
	*/
	windowBuffer[i] = weighingFactor;
	windowSum += weighingFactor;
  }
  
  for (uint16_t i = 0; i < (this->_samples >> 1); i++){
	  this->_vReal[i] *= windowBuffer[i] / windowSum;
	  this->_vReal[this->_samples - (i + 1)] *= windowBuffer[i] / windowSum;
  }
...
#warning("This method is deprecated and may be removed on future revisions.")
  ...
  //ajout d'enregistrement des poids de la fenêtre d'analyse et de la somme du buffer pour la normaliser - Modif
  //const int bufferSize = ;
  double windowBuffer[samples >> 1];
  double windowSum = 0.0;
  ...
  for (uint16_t i = 0; i < (samples >> 1); i++){
	  vData[i] *= windowBuffer[i] / (windowSum*2);
	  vData[samples - (i + 1)] *= windowBuffer[i] / (windowSum*2);
  }
}

void arduinoFFT::MelCoefficients(uint16_t windowSize, uint16_t nbFilters, uint16_t sampleRate, double[] buffer, uint16_t samples) {
	
	//Power Spectrum
	for (uint16_t i = 0; i < (samples >> 1); i++){
		buffer[i] = pow(buffer[i], 2)/windowSize;
	}
	
	
	
	uint16_t nqyst = sampleRate/2;
	
	float scale[nbFilters];
	
	float melScale = 2595*math.log10(1+nqyst/700);
	
	float hertz[nbFilters];
	
	float bins[nbFilters];
	
	scale[0] = 0;
	hertz[0] = -1;
	bins[0] = (windowSize + 1) * hertz[0] / sampleRate;

	
	for (uint8_t i=1; i<nbFilters-1; i++){
		scale[i] = i * melScale / (nbFilters-1);
		hertz[i] = (700 * (10**(scale[i] / 2595) - 1));
		bins[i] = floor((windowSize+1) * hertz[i] / sampleRate); //math.floor
	}
	
	int fbank[nbFilters][int(floor(windowSize/2))];
	
	for (uint8_t i=1; i<nbFilters-1; i++){
		int fmMinus = int(bins[i-1]);
		int fm = int(bins[i]);
		int fmPlus = int(bins[i+1]);
		
		for (uint8_t k=fmMinus; k<fmPlus; k++){
			fbank[i - 1, k] = (k - bins[i - 1]) / (bins[i] - bins[i - 1]);
		}
		for (uint8_t k=fmPlus; k<fmPlus; k++){
			fbank[i - 1, k] = (bins[i + 1] - k) / (bins[i + 1] - bins[i]);	
		}
	}
	float filterBanks[samples >> 1][int(windowSize/2)];
	for (uint16_t i = 0; i<(samples >> 1); i++){
		float res = 0;
		for (uint16_t j = 0; j<int(windowSize/2); j++){
			for (uint8_t k = 0; k<nbFilters; k++){
				res += fbank[] * buffer[]//...
			}
		}
	}
