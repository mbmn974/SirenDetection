...
#warning("This method is deprecated and may be removed on future revisions.")
  ...
  for (uint16_t i = 0; i < (samples >> 1); i++){
	  vData[i] /= windowSum*2;
	  vData[samples - (i + 1)] /= windowSum*2;
  }
}

void arduinoFFT::MelCoefficients(double* buffer, uint8_t nbFrames, uint16_t windowSize, uint16_t nbFilters, uint16_t sampleRate) {
	
	//Power Spectrum
	for (uint8_t n = 0; n < nbFrames; n++){
		for (uint16_t i = 0; i < (windowSize >> 1); i++){
			buffer[n * (windowSize >> 1) + i] = pow(buffer[n * (windowSize >> 1) + i], 2) / windowSize;
		}	
	}
	
	float* bins = new float[nbFilters+2];
	
	if(true){//to manage memory allocation of temporary variable (lazy)
		float nqyst = sampleRate/2;
		
		float melScale = 2595*log10(1+nqyst/700);
		
		float distance = melScale/(nbFilters+1);
		
		bins[0] = 0;
		
		for (uint8_t i=1; i<nbFilters+2; i++){
			bins[i] = floor((windowSize+1) * (700 * (pow(10,((i*distance) / 2595)) - 1)) / sampleRate);
		}
	}
	
	float fbank[nbFilters][windowSize >> 1] = {};
	
	for (uint8_t k=1; k<nbFilters + 1; k++){
		int fmMinus = int(bins[k-1]);
		int fm = int(bins[k]);
		int fmPlus = int(bins[k+1]);
		
		for (int i=fmMinus; i<fm; i++){
			fbank[k - 1][i] = (float(i - bins[k - 1])) / (float(bins[k] - bins[k - 1]));
		}
		for (int i=fm; i<fmPlus; i++){
			fbank[k - 1][i] = (float(bins[k + 1] - i)) / (float(bins[k + 1] - bins[k]));
		}
	}
	
	delete[] bins;

	float* filterBanks = new float[nbFrames * nbFilters];
	for (uint8_t n = 0; n<nbFrames; n++){
		for (uint8_t k = 0; k<nbFilters; k++){
			float res = 0;
			for (uint16_t i = 0; i<(windowSize >> 1); i++){
				res += float(fbank[k][i] * buffer[n * (windowSize >> 1) + i]);
			}
			if(res == 0){
				res = 0.000000000000001;
			}
			filterBanks[n * nbFilters + k] = 20 * log10(res);
		}
	}
	
	for (uint8_t n = 0; n<nbFrames; n++){
		for (uint8_t k = 0; k<nbFilters; k++){
			buffer[n * (nbFilters) + k] = filterBanks[n * (nbFilters) + k];
		}
	}
